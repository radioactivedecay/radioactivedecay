...
def build_nuclide_string(Z: int, A: int = None) -> str:
    """
    Converts an atomic number to nuclide symbol string. If an atomic mass is included,
    the full nuclide string will be returned.

    Parameters
    ----------
    Z : int
        Atomic number of the desired element/nuclide. 
    A : int, optional
        Atomic mass of the desired nuclide.
        
    Returns
    -------
    str
        Element or nuclide string corresponding to the input Z and A values.
    ...
    Examples
    --------
    >>> rd.utils.build_nuclide_symbol(8)
    'O'
    >>> rd.utils.build_nuclide_symbol(8,16)
    'O-16'
    ...
    if Z not in conv.keys():
        raise ValueError(
            str(Z) + ' is not a valid atomic number'
        )

    return_string = conv[Z]
    if A != None:
        return_string += "-" + str(A)
